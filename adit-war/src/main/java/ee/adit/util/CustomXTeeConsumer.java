...
public class CustomXTeeConsumer {
	...
	private static Logger LOG = Logger.getLogger(CustomXTeeConsumer.class);
	...
	private <T> Object sendRealRequest(T t, XTeeServiceConfiguration xteeServiceConfigurator, Collection<XTeeAttachment> attachments) {
		LOG.debug("Sending request using CustomXTeeConsumer...");
		return webServiceTemplate.marshalSendAndReceive(serviceConfiguration.getSecurityServer(), t, getCurrentMessageCallbackFactory().create(xteeServiceConfigurator, attachments));
	}
