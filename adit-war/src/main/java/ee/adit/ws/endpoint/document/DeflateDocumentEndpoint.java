...
public class DeflateDocumentEndpoint extends AbstractAditBaseEndpoint {
	...
	private static Logger logger = LogManager.getLogger(DeflateDocumentEndpoint.class);
	...
	protected Object v1(Object requestObject) {
		...
		try {
			...
			this.getDocumentService().addHistoryEvent(applicationName, documentId, user.getUserCode(),
					DocumentService.HISTORY_TYPE_LOCK, xroadRequestUser.getUserCode(), xroadRequestUser.getFullName(),
					DocumentService.DOCUMENT_HISTORY_DESCRIPTION_LOCK, user.getFullName(), requestDate.getTime());

			// Set response messages
			response.setSuccess(new Success(true));
			messages.setMessage(
					this.getMessageService().getMessages("request.deflateDocument.success", new Object[] {}));
