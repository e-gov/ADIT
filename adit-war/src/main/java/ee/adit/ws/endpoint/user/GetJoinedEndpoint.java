...
public class GetJoinedEndpoint extends AbstractAditBaseEndpoint {
    ...
    private static Logger logger = LogManager.getLogger(GetJoinedEndpoint.class);
        ...
        try {
            ...
            if (applicationRegistered) {
                ...
                if (accessLevel >= 1) {
                    ...
                    // Kontrollime, kas küsitud kirjete arv jääb maksimaalse
                    // lubatud vahemiku piiresse
                    ...
                    if (maxResults.intValue() <= configurationMaxResults.intValue()) {
                        ...
                        // Teeme andmebaasist väljavõtte vastavalt offset-ile ja
                        // maksimaalsele ridade arvule
                        List<AditUser> userList = this.getUserService().listUsers(startIndex, maxResults);

                        if (userList != null && userList.size() > 0) {
                            logger.debug("Number of users found: " + userList.size());
                            ...
                            messages.setMessage(this.getMessageService().getMessages("request.getJoined.success",
                                    new Object[] {new Integer(userList.size()).toString() }));

                            // Additional information for request log
                            String additionalMessage = this.getMessageService().getMessage("request.getJoined.success",
                        ...
                        AditCodedException aditCodedException = new AditCodedException(
                                "request.getJoined.maxResults.tooLarge");
     ...
     */
    private String outputToFile(List<AditUser> userList) throws XmlMappingException, IOException,
