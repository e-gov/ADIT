...
<%@page import="ee.adit.util.MonitorResult" %>
...
<body>
    ...
    <table border=1>
        ...
        %><tr><td><%=result.getComponent()%></td><td><%=result.getStatusString()%></td><td><%if(result.isSuccess()) {%><%=result.getDurationString()%><%} else {%><%=result.getExceptionString()%><%}%></td></tr>
        <%}%>
    </table>
